...
exports.cancle_flight = async (req, res) => {
  ...
  try {
    ...
    user.presentbook = user.presentbook.filter((flightinfo) => flightinfo.Seat !== items.Seat);
    ...
    for (let i = 0; i < flight.Seats_count; i++) {
      if (flight.Seats[i].SeatStatus && flight.Seats[i].SeatUser != null 
        && flight.Seats[i].SeatChar == items.Seat[0] && flight.Seats[i].SeatIndex == items.Seat[1] ) { 
        const bookedUser = await User.findById(flight.Seats[i].SeatUser);
        if (bookedUser && bookedUser.email === user_info.email) {
    ...
    await flight.save();
    res.status(200).json({ message: 'Flight booking canceled successfully' });

  } catch (error) {
    console.error('Error during flight cancellation:', error);
    res.status(500).json({ message: 'Server error' });
  }
};